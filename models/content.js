...
Content.create = (newContent, result) => {
    sql.query('INSERT INTO contents (name, content_type_id, author_id, duration, link, is_active, description, image, created_at ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)',
             [newContent.name, newContent.contentTypeId, newContent.authorId, newContent.duration, newContent.link, 1, newContent.description, newContent.image, newContent.createdAt ], (err, res) => {
        if (err) {
...
Content.getById = (id, result) => {
    sql.query('SELECT contents.*, content_types.name AS content_type, ' +
...
Content.getAll = result => {
    sql.query('SELECT contents.*, content_types.name AS content_type, ' +
                'authors.name AS author_name FROM contents ' + 
                'INNER JOIN content_types ON content_types.id = contents.content_type_id ' +
                'INNER JOIN authors ON authors.id = contents.author_id ORDER BY contents.id ASC', (err, res) => {
        if (err) {
            console.log(err);
            result(null, err);
            return;
...
Content.getActive = result => {
    sql.query('SELECT contents.*, content_types.name AS content_type, ' +
                ...
                'INNER JOIN content_types ON content_types.id = contents.content_type_id ' +
                'INNER JOIN authors ON authors.id = contents.author_id ' +
                'WHERE contents.is_active = 1 ORDER BY contents.id ASC', (err, res) => {
        if (err) {
            console.log(err);
            result(null, err);
            return;
...
Content.update = (id, content, result) => {
    ...
    sql.query(
        ...
        [content.name, content.contentTypeId, content.authorId, content.duration, content.link, content.isActive,
            content.description, content.image, content.updatedAt, id],
        (err, res) => {
            if(err) {
                console.log('erros: ', err)
                result(err, null);
