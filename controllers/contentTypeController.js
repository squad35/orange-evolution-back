    ...
    ContentType.create(createContentType(req.body), (err, data) => {
        if (err){
            if(err.message.includes('null value'))
                res.status(400).send({message: 'os dados do tipo do conteúdo são obrigatórios e não podem ser nulos.'});
            else
                res.status(500).send({message: 'Houve um erro ao criar o tipo do conteúdo, por favor tente mais tarde!.'});
        }
        else
...
exports.deleteById = async (req, res) => {
    ...
    if(!req.body || req.body.id === '') {
        res.status(400).send({message: 'Para fins de exclusão, o id do tipo do conteúdo é obrigatório'});
    }

    //Salvando o novo usuário no banco de dados...
    ContentType.deleteById(req.body.id, (err, data) => {
        if (err){
            res.status(500).send({message: 'Houve um erro ao criar o tipo do conteúdo, por favor tente mais tarde!.'});
        } else {
            if(data.rowCount === 0) {
